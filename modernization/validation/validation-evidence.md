...
|---|---|---|---|---|
| F-1 | Command injection through `DBT_PROFILES_DIR`, MEDIUM, blocking | The value was interpolated into the three dbt recipe lines unquoted, so the shell parsed it: `DBT_PROFILES_DIR='/tmp; touch <marker>' make dbt` created the marker | Every payload is refused before any recipe runs, and nothing is executed while the file is read: the shell-metacharacter payload gives `*** DBT_PROFILES_DIR is set to "..." which carries ";"` and the Make-reference payload gives `*** DBT_PROFILES_DIR is supplied as "$(shell touch ...)", which carries a dollar sign`, both exit **2** with no marker created; the same `$(shell ...)` payload was driven through all ten caller-supplied values, from the environment and from the command line, each refused with nothing created; a legitimate value renders as `--profiles-dir '/tmp/dbt_profiles_clone1'`, single-quoted | `CALLER_ACCEPTED_CHARACTERS`, `check_caller_word` and `check_caller_words` of `modernization/Makefile`, applied to `DBT_PROFILES_DIR`, `CASE`, `CASES`, `CASES_MODE`, `SOURCE_SYSTEM_KEY`, `EXTRACT_DATE`, `STAGE`, `COBC` and `COBFLAGS` (`D-116`), with `CALLER_SUPPLIED_VALUES` and `check_caller_literal` measuring the supplied text of all ten plus `DBT_TARGET` before the file expands any of them (`D-129`) |
...
**Two controls were narrowed during this re-verification, not before it.** Re-executing the reproductions against the
reconciled tree measured two gaps the first pass had left, and both were closed and re-measured here. The write
confinement of F-2 and F-3 accepted a destination below the temporary directory that stood inside another repository
checkout, which matters on a host where every checkout stands below `/tmp`: a report aimed at `../base/src/qa.md` was
written into the shared workspace directory beside the checkouts, and a path inside a sibling clone was accepted
outright. Both tools now refuse a destination inside the directory holding this checkout and inside any other checkout,
recognised by a `.git` entry on the chain (`D-128`). The caller-value grammar of F-1 measured the **expanded** value, and
Make expands a reference inside a value it imports: `DBT_PROFILES_DIR='$(shell touch <path>)'` ran `touch` while the file
was being read, the check then saw the empty string the reference expanded to and accepted it, and the same payload in
`DBT_TARGET` passed unnoticed. The supplied text of all ten caller values plus `DBT_TARGET` is now measured before the
file expands any of them (`D-129`).
