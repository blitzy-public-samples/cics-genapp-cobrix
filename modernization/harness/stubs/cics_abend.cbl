      ...
      *
      * Order guard: this module carries no prerequisite. The six
      * command sites listed above are of two kinds - the two
      * zero-length COMMAREA checks at base/src/lgapol01.cbl:98-102
      * and base/src/lgapdb01.cbl:165-169, which precede every other
      * statement of their program, and the four product-insert failure
      * branches. The eight stubs whose sites carry an ordering
      * constraint of that source test it and write HC-ORDER-VIOLATION
      * and HC-ORDER-VIOLATION-STMT; this module reads neither item and
      * writes neither, so an abend leaves the order verdict of the
      * case exactly as the statements before it left it.
      *
      * Rationale belongs to modernization/docs/decision-log.md, rows:
