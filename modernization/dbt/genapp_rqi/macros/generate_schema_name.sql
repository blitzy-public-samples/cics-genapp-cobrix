{#
    ...
    Returned value:
      ...
      appended. The layer defaults of dbt_project.yml resolve to the literal
      schemas staging, intermediate and canonical, and each mart of
      models/marts/canonical lands in canonical under the alias it sets in its
      own config block.
    ...
    Scope:
      ...
      models/staging/genapp_class_exemplar/_genapp__sources.yml names its own
      schema and does not pass through this macro. The macro issues no DDL:
      the raw and canonical namespaces come from
      modernization/warehouse/ddl/01_schemas.sql, and dbt provisions staging
      and intermediate itself at run time.
